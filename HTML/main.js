...
app.post('/register', (req, res) => {
  const { name, email, phone, event_type } = req.body;

  db.query("INSERT INTO Users (full_name, email, city, registration_date) VALUES (?, ?, ?, CURDATE())",
    [name, email, "Unknown"], (err, result) => {
      if (err) throw err;
      const user_id = result.insertId;

      const event_id = event_type === "festival" ? 1 : (event_type === "cleanup" ? 2 : 3);
      db.query("INSERT INTO Registrations (user_id, event_id, registration_date) VALUES (?, ?, CURDATE())",
        [user_id, event_id], (err2) => {
          if (err2) throw err2;
